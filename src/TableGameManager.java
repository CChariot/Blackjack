...
public class TableGameManager implements gameProcess {
    private static TableGameManager obj;
    ...
    static synchronized TableGameManager getInstance() {
        if (obj == null) obj = new TableGameManager();
    ...
    public void playersHit(Player[] players_pool, Dealer myDealer, Deck currDeck) throws IOException {
        ...
        for (int i = 0; i < player_in; i++) {
            ...
            if (shouldSplit(players_pool[i])) {
                ...
                while (!players_pool[i].isFinal && TableStrategy.betting(currDeck, players_pool[i].split_game_value, myDealer)) {
                    players_pool[i].splitHit(currDeck);
                }
            }
            while (!players_pool[i].isFinal && TableStrategy.betting(currDeck, players_pool[i].game_value, myDealer)) {
                players_pool[i].hit(currDeck);
            }
        }
    }
