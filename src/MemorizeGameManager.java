...
public class MemorizeGameManager implements gameProcess {
    private static MemorizeGameManager obj;
    ...
    //constructor
    public MemorizeGameManager() {
    }

    //Singleton Design Pattern
    static synchronized MemorizeGameManager getInstance() {
        if (obj == null) obj = new MemorizeGameManager();
    ...
    public void playersHit(Player[] players_pool, Dealer myDealer, Deck currDeck) {
        ...
        for (int i = 0; i < player_in; i++) {
            ...
            if (shouldSplit(players_pool[i])) {
                ...
                while (!players_pool[i].isFinal && MemorizeStrategy.betting(currDeck, players_pool[i].split_game_value, myDealer)) {
                    players_pool[i].splitHit(currDeck);
                }
            }
            while (!players_pool[i].isFinal && MemorizeStrategy.betting(currDeck, players_pool[i].game_value, myDealer)) {
                players_pool[i].hit(currDeck);
            }
        }
    }
